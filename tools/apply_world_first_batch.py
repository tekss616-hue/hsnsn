...
.world-game{position:fixed!important;inset:0!important;z-index:10000!important;width:100%!important;height:100%!important;min-height:100dvh!important;overflow:hidden!important;background:#070a0d!important;color:#fff!important;display:flex!important;flex-direction:column!important}
.world-game[hidden]{display:none!important}
.world-scene{flex:0 0 auto;height:clamp(235px,38vh,390px)!important;background-position:center 42%!important;background-repeat:no-repeat!important;background-size:cover!important;filter:saturate(.94) contrast(1.03)}
...
let preview=false,current='beach',counts={beach:3,palace:2,garden:1,hospital:2,police:0,market:1,restaurant:2,hotel:0,harbor:1},messages={},returnState=[];
...
function isolateWorld(){captureReturnState();document.body.classList.add('world-preview-active');document.querySelectorAll('main').forEach(m=>{if(m.id!=='worldGame')m.hidden=true});['settingsSheet','adminAiPanel','helpReportPanel','adminSection','adminPreviewMode'].forEach(id=>{const e=$(id);if(e)e.hidden=true})}
function openWorld(isPreview){preview=!!isPreview;current='beach';const g=$('worldGame');if(!g)return;isolateWorld();g.hidden=false;document.body.style.overflow='hidden';ensure(current);render()}
function closeWorld(){const g=$('worldGame');if(g)g.hidden=true;document.body.classList.remove('world-preview-active');document.body.style.overflow='';if(preview){const d=$('adminDashboard');if(d)d.hidden=false}else if(returnState.length){returnState.forEach(id=>{const e=$(id);if(e)e.hidden=false})}returnState=[]}
...
window.HSNSNWorld={open:()=>openWorld(false),preview:()=>openWorld(true),move,close:closeWorld};
})();
'''

idx.write_text(html,encoding='utf-8');cssf.write_text(css,encoding='utf-8');jsf.write_text(js,encoding='utf-8')
print('Applied approved 9 world images with full-screen isolated preview and clean return behavior')
