...
if 'function setSearchBusy(' not in s:
    ...
    helper='''function setSearchBusy(on){searchBusy=on;const b=$('friendSearchBtn');if(!b)return;b.disabled=on;b.classList.toggle('searching',on);b.innerHTML=on?'<span class="search-spinner" aria-hidden="true"></span><span>بحث</span>':'بحث'}function finishSearch(){clearTimeout(searchTimer);searchTimer=null;setSearchBusy(false)}'''
