...
for script in ['tools/apply_final_runtime_repairs.py','tools/apply_first_roleplay_batch_compat.py','tools/apply_friend_push_restore.py','tools/apply_friend_push_sender.py','tools/apply_second_roleplay_batch.py']:
    p=Path(script)
    if not p.exists():raise SystemExit(script+' missing')
    exec(compile(p.read_text(encoding='utf-8'),str(p),'exec'),{})
print('Applied final live-roleplay cleanup and roleplay batches')