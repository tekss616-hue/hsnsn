...
auth=r'''    public class AuthBridge {
        ...
        private boolean isUsernameConflict(Exception e){ return e instanceof com.google.firebase.firestore.FirebaseFirestoreException && ((com.google.firebase.firestore.FirebaseFirestoreException)e).getCode()==com.google.firebase.firestore.FirebaseFirestoreException.Code.ABORTED; }
        private String encodedDisplayName(String name,String username){ return name.trim()+AUTH_PROFILE_SEP+cleanUsername(username); }
        ...
        private Task<Void> claimProfileDocs(FirebaseUser u,String name,String username){
            ...
            return firestore.runTransaction(tr->{
                ...
                if(existing.exists()&&!u.getUid().equals(existing.getString("uid")))
                    throw new com.google.firebase.firestore.FirebaseFirestoreException("اسم المستخدم مستخدم بالفعل.",com.google.firebase.firestore.FirebaseFirestoreException.Code.ABORTED);
                Map<String,Object> idx=new HashMap<>();idx.put("uid",u.getUid());tr.set(uname,idx);
        ...
        private void sendProfileAuth(String action,FirebaseUser user,boolean hintedProfile){
            ...
            final String fallbackName=authVisibleName(user);final String fallbackUsername=cleanUsername(authEmbeddedUsername(user));ensureFirestore();
            firestore.collection("users").document(user.getUid()).get().addOnCompleteListener(t->{
                String name=fallbackName,username=fallbackUsername;
                if(t.isSuccessful()&&t.getResult()!=null&&t.getResult().exists()){
                    DocumentSnapshot d=t.getResult();String dn=str(d,"name",name),du=cleanUsername(str(d,"username",username));if(!dn.isEmpty())name=dn;if(!du.isEmpty())username=du;
                }
                final String finalName=name,finalUsername=username;
                if(finalUsername.isEmpty()){dispatchProfile(action,user,finalName,"",false,false);return;}
                firestore.collection("usernames").document(finalUsername).get().addOnCompleteListener(idxTask->{
                    if(idxTask.isSuccessful()&&idxTask.getResult()!=null){
                        DocumentSnapshot idx=idxTask.getResult();String owner=idx.exists()?idx.getString("uid"):null;
                        ...
                        claimProfileDocs(user,finalName.isEmpty()?"المحقق":finalName,finalUsername).addOnCompleteListener(claim->{
                            if(claim.isSuccessful())dispatchProfile(action,user,finalName,finalUsername,true,false);
                            else if(isUsernameConflict(claim.getException()))dispatchProfile(action,user,finalName,finalUsername,false,true);
                            else dispatchProfile(action,user,finalName,finalUsername,!fallbackUsername.isEmpty()||hintedProfile,false);
                        });
                    }else dispatchProfile(action,user,finalName,finalUsername,!finalUsername.isEmpty(),false);
                });
            });
        }
        private void finishProfileAuth(FirebaseUser user,String name,String username,String action){
            ...
            user.updateProfile(up).addOnCompleteListener(t->sendProfileAuth(action,user,true));
        }
        private void reserveAndSaveProfile(FirebaseUser user,String name,String username,String action){
            ...
            claimProfileDocs(user,name,un).addOnSuccessListener(v->finishProfileAuth(user,name,un,action)).addOnFailureListener(e->{
                if(isUsernameConflict(e))sendAuthResult(false,action,null,"اسم المستخدم مستخدم بالفعل. اختر اسمًا آخر.");
                else sendAuthResult(false,action,null,"تعذر حفظ هوية المحقق في الخادم. تحقق من الاتصال وحاول مرة أخرى.");
            });
        }
...
# Never expose the hidden username suffix as part of the visible investigator name.
...
p.write_text(s)
print('Applied strict durable auth profile patch')
